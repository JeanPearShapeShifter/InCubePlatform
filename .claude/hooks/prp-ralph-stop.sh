...
if [[ ! "$ITERATION" =~ ^[0-9]+$ ]]; then
  echo "Warning: PRP Ralph state file corrupted (invalid iteration)" >&2
  rm "$STATE_FILE"
  exit 0
fi

if [[ ! "$MAX_ITERATIONS" =~ ^[0-9]+$ ]]; then
  echo "Warning: PRP Ralph state file corrupted (invalid max_iterations)" >&2
  rm "$STATE_FILE"
  exit 0
fi
...
if [[ ! -f "$TRANSCRIPT_PATH" ]]; then
  echo "Warning: PRP Ralph transcript not found" >&2
  rm "$STATE_FILE"
  exit 0
fi
