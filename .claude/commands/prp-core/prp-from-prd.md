...
**Determine project**: Based on the feature/product discussed, determine which project this PRD belongs to:
- If it involves the backend API, AI agents, database, or Python services → `{project}` = `backend`
- If it involves the frontend UI, React components, Next.js pages → `{project}` = `frontend`
- If it spans both or is unclear, ask the user which project this PRD belongs to

**Output path**: `.claude/PRPs/prds/{project}/{kebab-case-name}.prd.md`

Create directory if needed: `mkdir -p .claude/PRPs/prds/{project}`
...
| Environment | Database | Connection | Data Strategy |
|-------------|----------|------------|---------------|
| Development | incube_dev | localhost:5434 (Docker) | Seed/mock data |
| Production | incube | Docker internal network | Real data |

- Dev compose: `docker-compose.dev.yml`
- Migrations: Alembic - run in dev first, apply to production during deploy
- Env files: `.env.example` (template) / `.env` (local, never in git)
...
{Explain which phases can run in parallel and why, e.g., "Phases 3 and 4 can run in parallel in separate worktrees as they touch different domains (frontend vs backend)"}
...
**File**: `.claude/PRPs/prds/{project}/{name}.prd.md`
...
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│  GENERATE: Write PRD to .claude/PRPs/prds/{project}/    │
└─────────────────────────────────────────────────────────┘
```
