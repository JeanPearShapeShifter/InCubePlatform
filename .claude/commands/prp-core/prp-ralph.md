...
```bash
# Backend validation (adapt to plan)
cd backend && source .venv/bin/activate && ruff check . && pytest
# Frontend validation (adapt to plan)
...
- Build: PASS/FAIL
- Browser: PASS/FAIL/SKIPPED ({details if failing, SKIPPED if not a UI phase})
...
- [ ] Tests pass
- [ ] Build succeeds
- [ ] Browser validation passes (if UI phase): no console errors, routes load, responsive layouts intact
...
1. **Generate Implementation Report**
   ...
   Determine `{project}` from the plan path (contains `/backend/` → backend, `/frontend/` → frontend, or infer from plan name prefix).

   Create `.claude/PRPs/reports/{project}/{plan-name}-report.md`:
...
2. **Archive the Ralph Run**
   ...
   cp .claude/PRPs/reports/{project}/{plan-name}-report.md "$ARCHIVE_DIR/learnings.md"
   ```
...
4. **Archive Plan to Completed**
   ...
   mv {plan_path} .claude/PRPs/plans/{project}/completed/
   ```
